...
From Lang        Require Import Semantics AST Theory.
From Compiler    Require Import Compiler Renv LLVMUtils.
...
Fixpoint add_variables_asl (vars : list dec) (e:env) : (env * unit) :=
match vars with
| nil => (e, tt)
| h :: t => match h with
            | Var x =>
...
Fixpoint add_variables_asl_itree {E} (vars : list dec) (e:env) : itree E (env * unit) :=
match vars with
| nil => Ret (e, tt)
| h :: t => match h with
            | Var x =>
...
Definition preallocated (ss : list stmt) (l:local_env) (g:global_env) m e : Prop :=
forall x exp,
In (Assign x exp) ss ->
(exists ptr,
allocated ptr m /\
...
forall ss l m g e a,
preallocated (a :: ss) l g m e ->
preallocated ss l g m e.
Proof.
induction ss.
+ intros.
...
preallocated ss l g m e ->
Renvh e g l m ->
eutt (eutt_inv TT) (interp_asl (denote_stmts ss) e) (⟦ compile_stmts ss ⟧c3 g l m).
Proof.
induction ss.
+ intros.
  subst.
...
+ destruct a.
  - intros.
    ...
    destruct (in_dec stmt_dec (Assign x (Lit i)) (Assign x (Lit i) :: ss)).
    -- apply H in i0.
       destruct i0.
       destruct H1 as [ptr].
       destruct H1.
...
Theorem compiler_correct (ds:list dec) (ss:list stmt) :
  well_formed ds ss ->
  bisimilar TT (denote_decs ds ;; denote_stmts ss) (denote_code ((compile_decs ds) ++ (compile_stmts ss))).
Proof.
  unfold bisimilar.
  intros.
  ...
  assert (HPrealloc : preallocated ss l' g' m' e').
  apply (add_variables_prealloc l_llvm m_llvm g_llvm g_asl ds ss l' m' g' e' H Eqn1 Eqn2).
