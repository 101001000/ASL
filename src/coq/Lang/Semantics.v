...
Section Denote.
  ...
  Definition denote_expr (e : expr) : itree eff int32 :=
    match e with
    | Lit n     => ret n
    end.

  Definition denote_stmt (s : stmt) : itree eff unit :=
    match s with
    | Assign x e =>  v <- denote_expr e ;; trigger (SetVar x v)
    | Skip => ret tt
    end.

  Fixpoint denote_stmts (ss : list stmt) : itree eff unit :=
    match ss with
      | nil => ret tt
      | h :: t => denote_stmt h ;; denote_stmts t
    end.


  Fixpoint denote_decs (ds : list dec) : itree eff unit :=
    match ds with
      | nil => ret tt
      | h :: t => match h with | Var x => trigger (SetVar x (Int32.repr 0%Z)) end ;; denote_decs t
    end.

  Definition denote_prog (p : prog) : itree eff unit :=
    match p with
      | Prog ds ss => denote_decs ds ;; denote_stmts ss
    end.
